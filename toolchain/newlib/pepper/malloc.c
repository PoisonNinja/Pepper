...
#ifdef DEBUG
    printf("Found tag with %i bytes available (requested %i bytes, leaving "
           "%i), which has exponent: %i (%i bytes)\n",
...
#ifdef DEBUG
            printf("Old tag has become %i bytes, new tag is now %i bytes (%i "
                   "exp)\n",
...
void __attribute__((malloc)) free(void *ptr)
{
    int index;
    struct boundary_tag *tag;
...
#ifdef DEBUG
        printf("Melting tag left into available memory. Left was %i, becomes "
               "%i (%i)\n",
...
#ifdef DEBUG
        printf("Melting tag right into available memory. This was was %i, "
               "becomes %i (%i)\n",
...
void *__attribute__((malloc)) realloc(void *p, size_t size)
{
    ...
    tag =
        (struct boundary_tag *)((unsigned int)p - sizeof(struct boundary_tag));
    real_size = tag->size;
...
void *_calloc_r(struct _reent *ptr, size_t nmemb, size_t size)
{
